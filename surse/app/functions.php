...
class functions{
    ...
    public function advencryptor($string) {
        //Functie de encodare pentru parolele de administrare
        //Truncare pt protectie flood
        return sha1(md5(substr($string,0,30)));
    }

    public function secure($text, $chk = 1) {
        global $db, $userip;
        if ($chk == 1) {
            $pot = array("'", '"', " OR ", " or ");
        } elseif ($chk == 2) {
    ...
    public function truncate($text, $len = 70) {
    ...
    public function vot() {
        ...
        if ($chk->num_rows == 1) {
            ...
            $tm = time();
            $chk2 = $db->query("SELECT count(1) FROM sesiuni_vot WHERE id=$ssid AND inceput<$tm AND incheiere>$tm LIMIT 1")->fetch_array()[0];
            //Verificam daca aceasta sesiune de vot este deschisa si se poate vota
            if ($chk2 == 1) {
                $cnp = $_SESSION['cnp'];
                $chkimp = $db->query("SELECT count(1) FROM voturi WHERE sesiune_vot=$ssid AND cnp=$cnp")->fetch_array()[0];
    ...
    public function reclamatie() {
        ...
        if (!(isset($_POST['reclamatie']) && isset($_POST['tel']))) {
            ...
            $this->secure($_POST['tel'] . $_POST['reclamatie'], 2);
    ...
    public function admin_bec_check() {
        global $db;
        //Functie de autentificare in panoul de administrare bec
        if (isset($_POST['pass1']) && isset($_POST['pass2'])) {
            $p1 = $this->advencryptor($_POST['pass1']);
            $p2 = $this->advencryptor($_POST['pass2']);
            ...
            $p1 = $this->advencryptor($_SESSION['becss1']);
    ...
    public function admin_prim_check(){
        global $db;
        ...
        if (isset($_POST['pass1']) && isset($_POST['postal'])) {
            ...
            $p2 = $this->advencryptor($_POST['pass1']);
            //Daca au fost trimise parolele, se encripteaza
    ...
    public function admin_urna_check(){
        global $db;
    ...
    public function admin_dev_check() {
        global $db;
        ...
        if (isset($_POST['pass1']) && isset($_POST['pass2'])) {
            ...
            $p1 = $this->advencryptor($_SESSION['admss1']);
            //Daca exista setata o sesiune, encriptam parola, si verificam in baza de date
            $c1 = $db->query("SELECT 1 FROM setari WHERE nume='admin-dev-pass-1' AND valoare='$p1' LIMIT 1")->num_rows;
            if ($c1 == 1) {
                return "1";
    ...
    public function sendmail($tip, $email, $data=""){
        global $db;
